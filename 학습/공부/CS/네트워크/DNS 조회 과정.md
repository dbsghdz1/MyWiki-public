...
### 2026-09-24 (이어서) — messwithdns.net으로 장부 주인 쪽에서 보기

- 맥락: [messwithdns.net](https://messwithdns.net/)(Julia Evans)에서 서브도메인을 받아 `hello` A 레코드를 직접 만들고, 오른쪽 Requests 로그로 **누가 내 장부를 보러 오는지** 관찰
- 예측 ①: 로그의 요청자 IP는 내 맥 IP일 것 → **틀림.** 찾아온 건 `KIXS-AS-KR Korea Telecom (210.113.99.x)` — 확인자가 대신 찾아온다. 게다가 내가 묻는 창구(`168.126.63.1`)와 밖으로 나가는 기계(`210.113.99.1·.2·.6`)가 다르다. A 질의 1초 전에 `witch457` NS 질의가 먼저 찍힌다(한 단계씩 되묻기)
- 예측 ②: 다시 물으면 캐시돼서 로그가 안 생길 것 → **반쯤 맞음.** 로그가 계속 생겼다. KT는 주소 하나 뒤에 기계가 여럿이고 **캐시가 기계마다 따로**다. 캐시 적중 한 번은 8ms·TTL 3410 — 3600-3410=190초 전 = `.6`이 처음 캐시한 13:31:39와 정확히 일치
- 예측 ③: IP를 `1.2.3.4`→`5.6.7.8`로 바꾼 뒤 → 장부 주인 직접 질의(`@mess-with-dns1.wizardzines.com`)는 즉시 `5.6.7.8`, KT는 `1.2.3.4`(TTL 3483)와 `5.6.7.8`(TTL 3600)이 섞여 나옴, 구글 `8.8.8.8`은 `5.6.7.8`. **"DNS 전파" = 퍼지는 게 아니라 확인자 캐시가 제각각 만료되길 기다리는 것**
- 예측 ④: TTL을 30으로 낮춰도 이미 3600으로 캐시한 기계는 옛 답을 줄 것 → 맞음(낮춘 사실 자체를 모른다)
