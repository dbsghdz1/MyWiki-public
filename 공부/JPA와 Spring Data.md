...
- 근거: 커밋 `0cdeacf`(V3 CHECK 제약 + `DocumentRuleEnumConstraintTest`), PR #22. DB 쪽 문법은 [[공부/데이터베이스|데이터베이스]].
...
- 배운 것:
  ...
  - 그래서 제약 위반을 예외 타입으로 구분하려 할 때 **DB마다·제약 종류마다 다르다는 것을 전제**해야 한다. 상위 타입(`DataAccessException`)으로 받고 원인 메시지를 보는 쪽이 안전하다 — 대신 그러면 단언이 헐거워지므로 [[공부/테스트 코드|테스트 코드]]의 주의가 함께 필요하다.
