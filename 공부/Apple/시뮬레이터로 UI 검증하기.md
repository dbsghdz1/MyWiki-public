...
- **이게 오진의 원인이 된다.** 코드를 고치고 재실행했는데 옛 다이얼로그가 그대로 있으면, 고친 코드가 안 먹은 것으로 읽는다. 실제로 [[공부/Apple/SwiftUI|SwiftUI]]에 "TabView가 안 보이는 탭도 미리 로드해 권한을 묻는다"는 **틀린 결론**을 남긴 적이 있다 — 로그를 찍어 보니 그 `.task`는 아예 실행되지 않았다.
