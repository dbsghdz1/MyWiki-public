...
- **1겹에서 이름을 바꾸면 안 된다.** TS엔 Swift `CodingKeys`가 없어서 `JSON.parse` 결과는 서버가 준 키 그대로다. `trade_price`를 `tradePrice`로 적어두면 값이 조용히 `undefined`가 된다 → [[공부/TypeScript 타입 시스템|타입은 런타임에 아무것도 검사하지 않는다]].
...
- 상태 코드 검사는 여전히 필수다 — [[공부/TypeScript 타입 시스템|fetch는 404·500에 reject 하지 않는다]].
...
ticker 5초 · 일봉 60초 · 마켓 목록 3600초. 임의의 숫자가 아니라 각 리소스가 실제로 바뀌는 주기에서 나온다. → [[공부/Next.js 서버와 캐싱|Next.js 서버와 캐싱]]
