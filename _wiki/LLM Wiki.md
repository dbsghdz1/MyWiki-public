...
요약, 원문, 댓글, 에이전트의 해석은 근거 수준이 다르다. 원문을 직접 확인하지 않은 동안은 이를 페이지 상태와 안내문으로 드러내야 한다 — 이 페이지도 사용자 요약 한 건만 근거일 때는 `needs-review`였고, 2026-08-22 원문 gist를 확인해 해제했다(위 근거 상태 콜아웃). 원문이 추가되어도 기존 사용자 요약은 삭제하지 않고 별도 출처로 대조한다.
