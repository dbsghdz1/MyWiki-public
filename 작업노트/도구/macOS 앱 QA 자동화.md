...
- **매 조작 직전에 창 좌표를 다시 읽는다.** 창은 세션 중에 움직인다(앱 재실행, 시스템의 창 재배치). 원점을 한 번 읽어 두고 계속 쓰면 클릭·호버가 전부 엉뚱한 곳으로 가는데, 화면에는 "버튼이 반응하지 않는다"로 보여 **앱 버그로 오진한다.** 좌표 캐시가 아니라 매번 `CGWindowListCopyWindowInfo`.
- **앱 실행 직후 창이 바로 안 뜬다.** 데이터가 많으면 5초로도 모자라 `winid`가 빈 줄을 준다. 고정 `sleep` 대신 창이 잡힐 때까지 도는 루프로 기다린다.
- **호버로만 나타나는 요소(⋮ 등)를 클릭하려면 그 직전 조작까지 포커스가 유지돼야 한다.** 캡처가 포커스를 뺏은 뒤 활성화 없이 클릭하면 호버가 풀려 요소가 사라지고, 클릭은 그 밑의 행에 들어간다.
...
### 2026-09-01 — 창이 움직여서 30분을 오진했다 (탭탭)

- 맥락: 탭탭 macOS 디자인 QA(이슈 #143)에서 카테고리 ⋮ 팝업의 호버·클릭을 검증하다가.
- 배운 것: 창 원점을 한 번 읽어 두고(864,432) 계속 썼는데 중간에 창이 (225,653)으로 옮겨 갔다. 이후 모든 클릭·호버가 창 밖/엉뚱한 위치로 가서 **"팝업 호버가 안 걸린다", "팝업 항목 클릭이 먹지 않는다"**로 보였고, `overlayPreferenceValue` 오버레이의 히트 테스트 문제라고 결론 낼 뻔했다. 좌표를 다시 읽자 호버·클릭 모두 정상이었다(진짜 결함은 색 대비였다 — [[작업노트/Apple/SwiftUI|SwiftUI]] 2026-09-01 항목).
- 근거: `winid`로 같은 창(id 14746)의 원점이 세션 중에 바뀐 것을 확인. 재측정 후 `카테고리 편집하기` 클릭 → 수정 모달이 정상적으로 떴다.
