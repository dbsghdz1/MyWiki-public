...
- **Orca(stably.ai) 터미널 다크 테마는 설정값 `terminalThemeDark: Ghostty Default Style Dark`다** — Orca 내장 프리셋 이름이고, Orca가 Ghostty config를 읽는지는 확인 안 됨(cmux와 다름). Orca 쪽 설정 변경(`orca-data.json`·`orca computer` UI 조작)은 [[작업노트/도구/Orca 설정 변경|Orca 설정 변경]] (2026-09-22)
