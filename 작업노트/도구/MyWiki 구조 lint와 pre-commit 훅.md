...
- **`visibility: private` 파일로 가는 링크는 퍼블리시가 알아서 푼다**(2026-09-27 `publish.sh` 3·4단계). 전에는 `publish.deny`에 같은 경로를 손으로 적어야 했고, 빠뜨리면 링크가 공개판에서 깨진 채 나갔다. 6b 깨진 링크 게이트도 **경로가 든 링크는 그 경로로 끝나는 파일만** 인정한다 — 파일명만 비교하던 폴백이 다른 폴더의 같은 이름 파일(`학습 계획.md` 5개)에 걸려 깨진 링크를 통과시켰다.
...
### 2026-09-27 — `학습/공부/학습 계획.md` 링크 20개가 공개판에서 깨져 있었는데 게이트가 전부 통과시켰다

- 맥락: 홍 「위키 점검 — public·private 관리, 누락, 개선점」 전수 감사(에이전트 274개, 발견 139건) 뒤 「바로 고칠 수 있는 것」 처리
- 배운 것:
  - **3단계(`visibility: private` 제거)는 파일만 지우고 언링크 목록(`UNLINK_LIST`)에 넣지 않았다.** 목록은 `private_prefixes` + `publish.deny`로만 만들어져서, `publish.deny` 40행 주석(「자동 언링크가 안 된다 — 여기에도 적는다」)대로 손 등록이 필요했다. 09-18에 private로 바꾼 `학습/공부/학습 계획.md`가 등록에서 빠져 공개 문서 11개에 이 문서로 가는 위키링크 20개가 남았다. 수정: 3단계에서 지운 경로를 `PRIVATE_FILES`에 모아 `UNLINK_LIST`에 합친다.
  - **6b가 못 잡은 이유는 파일명 폴백.** 스테이징 경로 존재 검사(`-e "$STAGE/$t.md"`)가 실패하면 `find -name "$(basename "$t").md"`로 아무 폴더의 같은 이름 파일을 찾아 통과시켰다 — `학습 계획.md`가 CS 네 폴더와 JS/언어에 있었다. 수정: `t`에 `/`가 있으면 `find -path "*/$t" -o -path "*/$t.md"`(Obsidian의 부분 경로 해석과 같은 뜻), 없을 때만 파일명 비교. 같이 sed에 `s/\\$//`를 넣어 표 안 별칭 링크(`경로\|별칭`)에서 잘려 나온 끝 `\`를 벗겼다(그전에는 이 22개 링크도 폴백 덕에만 통과했다).
  - **파일 경로 항목은 표 안 `\|` 별칭에서 언링크가 안 됐다.** prefix 항목(`HSW` 등)은 `(?:/[^\]\|]*)?`가 `/README\`까지 먹어서 우연히 됐지만, 파일 경로 항목(`…/학습 계획`)은 뒤에 `/`가 없어 `\s*\|`에서 실패한다. 4a(표 행 삭제)는 `(?:[\/|\]]|\\\|)`, 4b(언링크)는 `\s*\\?\|`로 경계를 넓혔다.
  - 경고(6c)는 전체를 `head -30`으로 잘라서, 패턴 파일 순서상 뒤쪽인 Jira 키 84건·SNS URL 23건이 한 번도 화면에 안 나왔다. 패턴별 건수 + 예시 3줄로 바꿨다(전체는 `MYWIKI_WARN_ALL=1`). 예시 추출을 `head -3`으로 쓰면 `pipefail`에서 SIGPIPE 위험이 있어 `awk 'NR<=3'`.
  - 라벨 `6e`가 두 번(마커 잔존·비공개 참조) 쓰여 두 번째를 `6f`로. 문서가 말하던 「차단 게이트 4개」는 09-23부터 5개였다.
  - `deny-block`의 제3자 실명은 한글 성명만 있어서 **로마자(폴더명 `<이름>-birthday`)·호칭형(이름+님)·멘토 실명**이 빠져 있었다. grep -E는 대소문자를 구분하므로 로마자는 `[Hh][Yy]…`처럼 글자마다 연다. 추가 전에 공개 스테이징본에서 0건인지 먼저 확인해야 퍼블리시가 멈추지 않는다.
- 근거: 수정 전후 스테이징 diff(바뀐 줄 17쌍, 전부 언링크, 삭제된 줄 0) · 공개판에서 이 문서로 가는 링크 20 → 0 · `python3 -m unittest discover -s scripts/tests` 29건 통과
...
- 배운 것:
  ...
  - **공개 저장소 이력 정리는 auto 모드가 막는다** — `git filter-repo`는 스크래치 클론에서도 「Git Destructive」로 거부됐고, 그 뒤엔 같은 저장소의 읽기 `git log`까지 막혔다. 검증을 내장한 스크립트(백업 번들 → 새 클론에서 `--replace-text` + `--invert-paths` → 최신 트리 해시가 전과 같은지·대상 문자열과 경로가 이력에 0인지 → 통과 시에만 force push)를 홍이 `!`로 실행했다. 306 → 304 커밋, 최신 트리 동일. **GitHub은 옛 커밋을 SHA 직접 접근으로 계속 보여준다** — 완전 삭제는 GitHub 지원팀 요청
  - 스크립트 함정: bash에서 `$n개`처럼 변수 바로 뒤에 한글을 붙이면 한글 바이트까지 변수 이름으로 읽혀 `set -u`에 `unbound variable`로 죽는다 — `${n}개`로 쓴다
