...
**AI(에이전트)가 작업하며 알게 된 것을 다음 작업의 AI를 위해 남기는 영역이다.** 홍은 다시 읽지 않는다. 2026-08-28에 `공부/`에서 분리했다 — 공부는 홍이 이해하는 것, 여기는 AI가 검색하는 것.

**판정 질문: "이 프로젝트를 다시 만질 때 필요한가?"** — 그렇다면 여기에 쓴다. "면접에서 물어볼 수 있나?"에 해당하는 개념·원리는 [[공부/README|공부]]로 보낸다.
