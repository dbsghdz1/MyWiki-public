...
- **호버 영역과 클릭 영역이 어긋나면 "버튼이 안 눌린다"가 된다.** `.buttonStyle(.plain)` 버튼의 히트 영역은 **라벨의 불투명 픽셀**뿐인데, 배경·호버·`contentShape`를 `Button` **바깥**에 붙이면 그 캡슐 전체에 호버가 걸린다 — 커서를 올리면 색이 변하니 눌릴 것처럼 보이지만 글자·아이콘을 정확히 찍지 않으면 아무 일도 안 난다(탭탭 검색 카테고리 버튼 실측: 호버는 발화, 클릭은 무시). **배경·패딩·`contentShape`는 label 안으로 넣는다.** 호버를 나중에 얹는 작업이 이 함정을 드러내 준다 — 원래도 클릭이 안 되던 자리가 이제 눈에 보인다.
- **떠 있는 버튼과 상단바 버튼을 한 줄로 맞추려면 두 쪽의 상단 패딩을 같게 둔다.** 탭탭 macOS는 사이드바를 접으면 콘텐츠 위에 사이드바 열기 버튼이 뜨는데(`padding(.top, 20)`), 화면 상단바가 `padding(.vertical, 20)`을 쓰는 `MacToolbar`가 아니면 20pt만큼 어긋나 보인다. 겹침은 좌측 패딩(접힘 시 72)으로, 어긋남은 상단 패딩으로 각각 맞춘다.
...
- **같은 날 뒤집혔다 — 이 그래프는 앱에 없다.** 고친 화면을 본 홍의 판정은 *"하 뭔가 별론데 … 주제 탭을 없애고 녹음 → 옵시디언만 해보자"*. `TopicsView.swift`·`TopicGraph.swift`는 **커밋 전에 삭제**했고 탭은 녹음·설정 둘만 남았다(`MeetingStore.topicEntries`·`TopicChips`·`FlowLayout`도 제거). 주제 추출·위키링크(주제명 이중대괄호) 내보내기는 그대로다. **교훈: 볼트로 내보내는 앱이 볼트 앱의 대표 화면을 흉내 내면, 잘 만들어도 "그건 옵시디언이 더 잘한다"로 끝난다.** 위 그래프·라벨 항목은 다른 화면에서 쓸 때 유효하다. **그래프 코드는 앱 저장소에 한 번도 커밋되지 않았다** — 되살리려면 이 노트를 보고 다시 쓴다.
