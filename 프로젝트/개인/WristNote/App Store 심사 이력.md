...
## 1.1.3 (5) — 2026-09-09 제출 · 「주제가 안 나온다」의 원인

홍 제보로 시작했다. 1.1.2(스크린샷만 고친 버전)는 아직 `WAITING_FOR_REVIEW`였으므로 취소하고 여기에 합쳤다 — 심사 한 바퀴를 아꼈다.

**재현**: 시뮬레이터에 전사를 주입(`WRISTNOTE_TRANSCRIPT`)해 요약 단계를 그대로 돌렸다.

| 전사 | 결과 |
|---|---|
| 500자 | 성공 · 주제 3개 |
| 2,900자(단일 패스) | 성공 · 주제 5개 |
| 6,199자, 내용 다양(3청크) | 성공 · 주제 7개 |
| **12,691자, 반복 많음(5청크)** | **실패 — `Detected content likely to be unsafe`, 요약·주제 전부 없음** |
| 같은 12,691자로 `extractTopics`만 | **성공 · 주제 6개** |

**길이가 원인이 아니다** — 다양한 6,199자는 같은 청크 경로로 통과했다. 온디바이스 모델의 안전 가드레일이 반복 많은 입력에 튀는 것이고, **한국어 전사가 뭉개지면 정확히 그런 텍스트가 나온다**(README 리스크 표의 "회의"→"매일" 사례). 즉 전사 품질 문제가 요약 실패로 번지고, 주제가 요약 안에만 있어서 함께 사라졌다.

**고친 것**
- `SummaryService.topicsOnlySummary` — 요약이 막히면 주제만 따로 뽑아 담는다. 마지막 행이 증명이다: 같은 전사에서 `extractTopics`는 통과한다(앞 3,000자·단일 패스)
- `backfillTopics` 조건을 `summary != nil`에서 **주제가 비어 있음**으로 넓혔다. 기존 조건은 요약이 실패한 회의를 영영 건너뛰었다 — **홍의 기존 회의는 앱을 다시 여는 것만으로 채워진다**
- 목록 배지: 요약이 없으면 초록 「완료」 대신 「요약 없음」. 전에는 왜 비었는지 알 방법이 없었다
- `Meeting.title` — 주제만 있는 요약(title 빈 문자열)에서 목록이 빈 줄이 되지 않게 폴백

**함께 막은 것 — 워치 녹음의 조용한 실패**
인터럽션 처리가 코드에 아예 없었다(`grep interruption` 0건). 통화·Siri로 세션이 끊기면 녹음은 멈추는데 화면 타이머는 `Text(timerInterval:)`(벽시계)라 계속 올라가고, 아이폰에 보내는 길이도 벽시계라 **10분짜리 파일이 45분 회의로 뜬다.** 시작 5초 성장 검사는 있었지만 중간에 멈추는 건 못 잡았다.
- `AVAudioSession.interruptionNotification` 구독 → 거기까지를 정상 종료로 마무리해 전송하고 손목에 사유 표시
- 30초 정체 감시
