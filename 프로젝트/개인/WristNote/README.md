...
- **1.1.0 (build 2)** — ✅ 2026-09-03 구현 완료, 실기기 검증 대기: 주제 추출·칩·주제 페이지 + **그래프 뷰**(Canvas force-directed) + **옵시디언 마크다운 자동 내보내기**(주제 위키링크) + **노션 전송**(토큰·키체인, 서버 없음) + **워치 컴플리케이션**(탭 즉시 녹음). 홍 실기기 확인 후 제출. 같은 날 오후 서명 검증: `fastlane ios archive` 성공, 위젯 App ID(`….watchkitapp.widget`)는 아카이브 중 포털에 **자동 등록 확인**(전 2개→후 3개, aside 실측) — 포털 쪽 남은 작업 없음
