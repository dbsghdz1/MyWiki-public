...
## 현재 카드
- **단계**: 운영
- **현재**: 1.0 App Store 출시(2026-09-03, 리젝 0회) · 1.1.0(build 2) 09-04 심사 제출 WAITING_FOR_REVIEW — [[프로젝트/개인/WristNote/App Store 심사 이력|심사 이력]]
- **다음 판정**: 1.1.0 심사 결과 + 워치 화면 꺼짐·손목 내림 상태 30~60분 백그라운드 녹음 지속 실측(1.0은 이 검증 없이 제출)
- **지금 할 일**: 실제 회의 1건으로 30분+ 백그라운드 녹음·배터리를 검증한다
- **미배포 수정 1건**: 워치 녹음 타이머 건너뜀 수정(커밋 `7e557ec`, 2026-09-05) — 심사 중인 1.1.0 build 2에는 안 들어갔다. 다음 빌드에 실린다
