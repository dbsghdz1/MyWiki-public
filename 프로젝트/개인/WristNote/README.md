...
## 현재 카드
- **단계**: 운영
- **현재**: 1.0~1.1.4 출시(리젝 0회) · **1.1.4(build 6) 2026-09-20 06:48 KST 출시**(`asc state`·iTunes lookup 2026-09-23 확인) — 주제 전면 제거 + 워치 타이머 「1초씩 안 올라간다」 대응 — [[프로젝트/개인/WristNote/App Store 심사 이력|심사 이력]]
- **다음 판정**: 실기기에서 워치 타이머가 매초 오르는지(첫 녹음 0:05 되감김 없음) 홍 확인
- **지금 할 일**: UI/UX 수정 범위를 홍과 정한다 (홍 2026-09-19 「UI/UX 수정해야 할 것 같아」)
...
**주제 전면 제거 (2026-09-19 홍 결정 — *"주제 일단 빼자"*)**: 09-16엔 탭만 없애고 추출·위키링크 내보내기는 남겼는데, 이번에 **추출·백필·마크다운 위키링크·노션 「주제:」 줄·설정 문구까지 전부 걷었다**(`SummaryService.extractTopics`·`topicsOnlySummary`·`MeetingStore.backfillTopics`·`MeetingSummary.topics` 삭제). 1.1.3이 저장한 «주제만 있는 빈 요약»은 `MeetingSummary.hasContent`로 걸러 빈 «핵심 요약» 섹션이 안 뜬다. 같은 날 워치 타이머 제보(1초씩 안 올라감)도 손봤다 — [[작업노트/Apple/SwiftUI|SwiftUI 작업노트]] 2026-09-19. 둘 다 1.1.4(build 6)로 2026-09-20 제출, 실기기 확인 전. 아래 09-16 문단의 "주제 추출은 유지"는 이 결정으로 대체됐다.

**1.2 방향 — 주제 탭을 버리고 «녹음 → 옵시디언»만 (2026-09-16 홍 결정)**: 인앱 주제 그래프는 주제가 쌓이면 덩어리가 됐고, 옵시디언풍으로 고쳐 보여줬더니 홍 판정은 *"하 뭔가 별론데 회의 그냥 주제 탭을 없애고 녹음 → 옵시디언만 해보자"*. **탭은 녹음·설정 둘만 남았다.** 주제 추출은 유지하되 화면에 안 보이고, 내보낸 마크다운의 위키링크(주제명 이중대괄호)로만 나간다 — 회의들을 엮어 보여주는 일은 옵시디언 그래프가 한다. 노션 전송은 유지(홍 선택). **앱의 정체성이 "회의 앱"에서 "옵시디언에 회의록을 넣어 주는 도구"로 좁아졌다.**
...
> [!note] 슬롯 배분과의 관계
> 개인 허브의 `paused` 원칙(슬롯 배분 결정)과 별개로 홍이 착수를 직접 지시했다(2026-08-31). AI의 사전 평가는 "아이디어 기록 + 스파이크만, 본격 착수는 보류"였으나 홍의 결정으로 착수. 다른 active 프로젝트(Fadeo·한능검·Zappy 런치)와 슬롯을 나눈다.
...
- [[작업노트/Apple/온디바이스 음성 인식과 번역|온디바이스 음성 인식과 번역]] — **FoundationModels 가드레일은 입력 «모양»에도 반응한다.** 같은 전사를 통째로 요약하면 막히고 앞 3,000자만 단일 패스로 물으면 통과한다. 요약 하나에 제목·개요·주제를 다 매달면 가드레일 한 번에 전부 사라진다
- [[작업노트/Apple/온디바이스 음성 인식과 번역|온디바이스 음성 인식과 번역]] — **파일 전사도 확정 결과가 도중에 흘러나와 `range.end ÷ 전체 길이`로 진행률을 낼 수 있다.** 엔진은 맥에서 230배속 이상이고 우선순위와 무관하다 → "전사가 느리다"면 백그라운드에서 멈추는지부터 의심 (2026-09-12)
...
- [[작업노트/Apple/SwiftUI|SwiftUI]] — 코덱 사다리 재시작이 화면 타이머의 시작 시각까지 되감는다(실기기 첫 녹음 0:05 → 0:00). 표시용 시각과 판정용 시각을 나누고, 통한 포맷은 `UserDefaults`에 기억한다 (2026-09-19)
- [[작업노트/Apple/온디바이스 음성 인식과 번역|온디바이스 음성 인식과 번역]] — **조각 요약은 조각마다 저장해야 한다.** 55,488자 전사는 모델 호출 19번인데 결과를 마지막에만 저장하면, 앱이 잠들 때마다 처음부터 다시 돌아 영영 «요약 중»에 머문다 (2026-09-16 홍 제보로 발견)
- [[작업노트/Apple/SwiftUI|SwiftUI]] — force-directed 그래프를 화면 상자에 가두면 주제가 늘수록 덩어리가 된다. 월드 좌표에서 펼치고 줌·이동으로 맞추고, 라벨은 우선순위 순으로 겹치면 건너뛴다 (2026-09-16 주제 탭 개편 — **같은 날 탭째 제거**, 아래 결정 참조. 교훈만 남는다)
