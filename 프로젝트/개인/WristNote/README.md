...
- **1.1.0 (build 2)** — ✅ 2026-09-03 구현(주제 추출·칩·주제 페이지 + 그래프 뷰 + 옵시디언 마크다운 자동 내보내기 + 노션 전송 + 워치 컴플리케이션), 위젯 App ID 포털 자동 등록 확인. **09-04 00:37 심사 제출 → 스크린샷 이중 업로드 정리 후 재제출, WAITING_FOR_REVIEW**. 스크린샷에 주제 그래프 추가(로케일당 6장), **기본 언어 en-US 전환**(한국 외 사용자는 영어 페이지) → [[프로젝트/개인/WristNote/App Store 심사 이력|심사 이력]]
- 1.2 후보 — 청크 요약 실측, 녹음 용량(µ-law 16KB/s → AVAudioEngine 소프트웨어 AAC), 콜드 스타트 반응 지연, 한국어 전사 정량 측정
