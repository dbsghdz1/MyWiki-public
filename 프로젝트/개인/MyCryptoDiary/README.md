...
- [[공부/JS/Feature-Sliced Design|Feature-Sliced Design]] — 계층·슬라이스·세그먼트, `index.ts` public API, Next.js 루트 `app/`(URL·예약 파일)과 `src/`(제품 구현)의 경계, `app/api`와 `src/**/api`의 차이
...
- [[공부/JS/Next.js 서버와 캐싱|Next.js 서버와 캐싱]] — 서버 컴포넌트·Route Handler, `app/api` HTTP 출입구와 `src/**/api` 내부 함수, `revalidate`, 환경변수 인라인, proxy의 요청 단위 실행
