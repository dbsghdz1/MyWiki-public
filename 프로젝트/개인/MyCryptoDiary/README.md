...
8월 잔여는 **소마 1순위**, MyCryptoDiary는 **9월(주간 2026-09-01)부터 본격 재개**한다 (본인 결정). D1 이후 6일 공백(08-21~26 커밋 0)은 우선순위 위반이 아니라 의도된 배치였다 — Zappy·Fadeo·한능검은 소마 작업 중 AI 대기시간의 자투리 작업이었고, **이 프로젝트는 학습 모드(직접 타이핑)라 자투리 시간에 진행할 수 없다.** 9월부터는 전담 집중 블록으로 D2(Neon + Drizzle)부터 이어간다. 2주 계획 기한(~08-28)은 폐기 확정, "12일치를 순서대로"(08-20 결정) 유지.
...
| D2 Neon + Drizzle 스키마 | 9/1 재개 예정 | **대기** — 브랜치 `feat/db-schema`만 생성, 커밋 0. 8월 잔여는 소마 1순위(2026-08-27 결정) |
