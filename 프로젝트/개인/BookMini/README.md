...
- 2026-09-23 — **09-12~19 7일 판정 결과(원래 카드의 09-19 판정, 09-23 lint에서 기록): 공백 없이 돌지 못했다.** 로컬 `(로컬 경로)` 기준 15분 넘는 하트비트 공백 26회 — 최장은 09-18 06:54~09-19 23:21(약 40시간, 부산 부재)과 09-15 17:55~09-16 22:39(약 29시간), 나머지는 대부분 새벽 1~7시간. 같은 로그에 `ssh: connect … Operation timed out`이 누적 305줄. 공백 원인(전원 분리·덮개·네트워크)은 로그만으로 가르지 않았다 — 같은 날 전원 정책을 「연결 중에만 방지」로 바꿔 판정 기준 자체가 바뀌었다(아래 항목).
