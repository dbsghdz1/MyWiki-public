...
- [[공부/AppStore/App Store 성장 도구|App Store 성장 도구]] — Mac 전용 앱이라 Apple Ads·인앱 이벤트·커스텀 제품 페이지를 못 쓴다. Pro 출시 때 쓸 수 있는 것은 피처링 노미네이션(최소 3주 전)과 프로모션 텍스트뿐.
- [[공부/Apple/macOS 메뉴바와 샌드박스|macOS 메뉴바와 샌드박스]] — 샌드박스는 AX를 통째로 막고, 메뉴바 아이템 식별은 CGWindowList + 화면 기록 권한(재실행 필요)으로만 가능. 디스플레이별 행·명명 규칙 차이 주의.
