...
> 다른 프로젝트에 슬롯을 몰아주기로 하면서 이 프로젝트는 보류로 전환했다. **제품이나 코드에 문제가 있어서가 아니라 슬롯 배분 결정이다** — MVP 화면은 전부 merge된 상태로 멈춰 있고, 그대로 이어서 재개할 수 있다.
> **재개 조건**: 소마 종료(2026-12) 이후 재판정. 근거: 계획 우선순위 표.
