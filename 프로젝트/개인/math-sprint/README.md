...
type: project
title: "math-sprint"
summary: "Apps in Toss용 짧은 두뇌 퀴즈 스프린트 게임의 작업 진입점"
status: paused
aliases:
  - math-sprint
  - toss-2048
...
## 현재 카드
- **단계**: 보류
- **현재**: 2026-08-18 보류 결정. 마지막 커밋 08-14 `71b0955`(30초 혼합 두뇌 퀴즈로 개편 — 07-31~08-14에 공유·광고·리더보드·왕관 퍼즐·게임 허브 커밋 8건). build·`.ait` 통과는 07-30 기준 검증이고 08-14 상태는 미검증
- **다음 판정**: 소마 종료(2026-12) 이후 슬롯 재배분 시 재판정
- **지금 할 일**: 없음 — 재개 시 첫 질문은 실제 출시 목표와 마감일
- **하지 않을 일**: 추가 문제 유형·새 게임(필요 여부 판단 전) — 공유·랭킹은 07-31·08-01 커밋에 이미 있다

Apps in Toss 환경에서 실행되는 60초 암산 게임이다. 현재 설명은 2026-07-29에 실제 코드, README, production build를 확인한 결과를 기준으로 한다.

> [!note] 2026-09-23 코드 확인 — 아래 07-29 설명 이후 바뀐 것
